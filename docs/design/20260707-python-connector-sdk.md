...
`conduit-commons`' `schema/avro` package (proven against Go-produced golden
bytes — see `tests/test_schema_avro.py`). **Still not implemented:** a
schema registry client (no `SchemaService` gRPC stubs exist in this repo)
and automatic schema subject/version metadata population — `conduit.record.
