...
- `conduit.schema.AvroSchema` (new optional extra, `conduit-connector-sdk[avro]`,
  ...
  with `conduit-commons`' `schema/avro` package (`hamba/avro/v2`) — proven
  against golden bytes an actual Go program produced, not a Python-only
  round-trip (`tests/test_schema_avro.py`, `tests/testdata/avro_golden.json`).
